...
---
## 5. Настройка валидации секретного токена через переменные Nginx

1. **Преобразование входящих HTTP-заголовков в переменные:**
   Любые входящие HTTP-заголовки от клиента (Telegram) автоматически преобразуются во внутренние переменные Nginx по следующему правилу:
   ...
   Например, заголовок секретного токена `X-Telegram-Bot-Api-Secret-Token` (указанный в параметре `secret_token` на шаге 4) становится переменной:
   ```nginx
   $http_x_telegram_bot_api_secret_token
   ```

2. **Защита вебхука на уровне Nginx через проверку переменной:**
   Ограничьте доступ для спамеров и неавторизованных запросов до того, как они дойдут до вашего бота. Добавьте проверку в блок `location /bot-webhook`, например:
...
3. **Проверка синтаксиса Nginx и перезапуск Nginx:**
   Для проверки синтаксиса Nginx используйте команду `sudo nginx -t`. Если всё в порядке, перезапустите Nginx командой `sudo systemctl reload nginx`.
...
* **`Connection timed out` / `Connection refused`:**  
  ...
  - Проверьте настройки брандмауэра и убедитесь, что порт 443 открыт: `sudo ufw status` (для открытия порта выполните `sudo ufw allow 443/tcp`). В облачных хостингах проверьте правила Firewall / Security Groups в веб-панели управления.

* **`SSL error: self signed certificate` / `certificate verify failed`:**  
...
* **`Wrong response code (502 Bad Gateway)`:**  
...
* **`Wrong response code (403 Forbidden)`:**  
...
* **`Wrong response code (504 Gateway Timeout)`:**  
  Таймаут ожидания ответа. Чтобы определить причину и верное решение, проверьте журналы Nginx (`/var/log/nginx/error.log` и `/var/log/nginx/access.log`):

  - **Случай 1 (В `error.log` есть ошибка `upstream timed out (110: Connection timed out)`):**  
    Nginx не получил ответ от локального приложения бота в течение 60 секунд.
    - Увеличьте значение `proxy_read_timeout` в конфигурации Nginx:
      ```nginx
      location /bot-webhook {
          proxy_pass http://127.0.0.1:8000;
      ...
      ```
    - Увеличьте время ожидания ответа в коде самого приложения бота.

  - **Случай 2 (В `access.log` код `499` или Telegram сбрасывает соединение и повторно шлёт запросы):**  
    Бот выполняет длительную операцию, из-за чего Telegram сам обрывает связь по своему таймауту (~60 сек) и начинает циклично слать запрос заново. Решения могут быть следующими:
